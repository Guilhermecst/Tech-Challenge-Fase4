...
# --- ABA 2: DASHBOARDS (CORRIGIDA E MELHORADA) ---
with tab2:
    ...
    # 2. DISTRIBUIÇÃO POR SEXO (PESO E IMC)
    ...
    ax1.axvline(mulheres['Peso'].mean(), color='red', linestyle='--', label=f'Média: {mulheres["Peso"].mean():.1f}')
    ax1.legend()
        
    sns.histplot(homens['Peso'], bins=20, kde=True, color='royalblue', alpha=0.7, ax=ax2)
    ax2.set_title('Homens')
    ax2.axvline(homens['Peso'].mean(), color='red', linestyle='--', label=f'Média: {homens["Peso"].mean():.1f}')
    ax2.legend()
    plt.tight_layout()
    st.pyplot(fig_sexo_peso)

    # 3. GRÁFICO DE BARRAS - NÍVEIS DE OBESIDADE
    ...
    ax_barras.set_xticklabels([label.replace('_', ' ').title() for label in ordem], rotation=45, ha='right')
    ax_barras.legend()
